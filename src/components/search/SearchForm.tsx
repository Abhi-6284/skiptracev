...
import { mergeJsonObjects } from "@/lib/file";
...
const SearchForm: React.FC = () => {
  ...
  const [batchFileData, setBatchFileData] = useState<string[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let response: any;
      if (searchMethod === "single") {
        response = await axios.post("http://127.0.0.1:8000/scrape", payload, {
          headers: {
      ...
      if (searchMethod === "batch") {
        response = await axios.post(
          "http://127.0.0.1:8000/file-scrape",
          {
            data: JSON.stringify(batchFileData),
          },
          {
            headers: {
              "Content-Type": "application/json",
        ...
        setResults(mergeJsonObjects(response.data.results));
        toast.success("Search submitted successfully!");
      }

      if (searchMethod === "paste") {
        const batchFileData = batchText.split("\n");

        console.log(batchFileData);
      ...
      reader.onload = (event: ProgressEvent<FileReader>) => {
        if (event.target?.result) {
          if (typeof event.target.result === "string") {
            setBatchFileData(event.target.result.split("\n"));
          } else {
